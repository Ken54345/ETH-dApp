...
const App = () => {
  ...
  const contractAddress = "0x901245C7F2e5eF4660a0a756a3a3Dd07BB5AF299";
  ...
  const wave = async () => {
    try {
      ...
      if (ethereum) {
        ...
        console.log("Conract balance before wave", ethers.utils.formatEther(contractBalance));
        /* コントラクトに👋（wave）を書き込む */
        ...
        const contractBalancePost = await provider.getBalance(wavePortalContract.address);
        console.log("Contract balance after wave:", ethers.utils.formatEther(contractBalancePost));
        // 残高が減っていればETHが送金されたことを確認
